        ...
        def ser():
            if (len(self.studid.get()) == 0):
                ...
                try:
                    ...
                    cursor.execute(
                        "Select bi.issue_id,s.name,b.name,bi.issue_date,bi.return_date from issue_book bi,student s,book b where s.stud_id = bi.stud_id and b.book_id = bi.book_id and s.stud_id = %s", &  # 91;change])
                    pc = cursor.fetchall();
                    if pc:
                        self.listTree.delete(*self.listTree.get_children());
                    for row in pc:
                        self.listTree.insert("", 'end', text=row &
                    # 91;0] ,values = (row&#91;1],row&#91;2],row&#91;3],row&#91;4]))
                    else:
            ...
            if (len(self.bookid.get()) == 0):
                ...
                try:
                    ...
                    self.myCursor.execute(
                        "Select bi.issue_id,s.name,b.name,bi.issue_date,bi.return_date from issue_book bi,student s,book b where s.stud_id = bi.stud_id and b.book_id = bi.book_id and b.book_id = %s", &  # 91;book])
        ...
        def check():
            try:
                ...
                if not z:
                    ...
                    self.brt = Button(self, text="Issue Book", width=15, font=('Courier new', 10), command=ib).place(
                        x=800, y=250)
                    self.brt = Button(self, text="Return Book", width=15, font=('Courier new', 10), command=ret).place(
                        x=1000, y=250)
